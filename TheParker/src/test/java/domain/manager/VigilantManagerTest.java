...
public class VigilantManagerTest {
	 ...
	 * Nested mockito
	 * 
	 * @Test public void registerVehicleInCarTest() {
	 * 
	 * // Arrange Car car = new CarDataBuilder().build(); Response response = new
	 * ResponseDataBuilder().build(); // Act
	 * Mockito.when(vigilantManager.registerVehicleIn(car)).thenReturn(response); //
	 * Assert assertEquals("Vehicle registration", response,
	...
	/*
	 * @Test public void getAvailableParkingCellsTest() { // Arrange Car car = new
	 * CarDataBuilder().build(); ParkingCell cellC01 = new
	 * ParkingCellDataBuilder().build(); ParkingCell cellC07 = new
	 * ParkingCellDataBuilder().withCode("C07").build(); ParkingCell cellC15 = new
	 * ParkingCellDataBuilder().withCode("C15").build(); ParkingCell cellC16 = new
	 * ParkingCellDataBuilder().withCode("C16").build(); ParkingCell cellC19 = new
	 * ParkingCellDataBuilder().withCode("C19").build(); List<ParkingCell>
	 * availableCells = new ArrayList<>(); availableCells.add(cellC01);
	 * availableCells.add(cellC07); availableCells.add(cellC15);
	 * availableCells.add(cellC16); availableCells.add(cellC19); // Act
	 * Mockito.when(vigilantManagerImpl.getAvailableParkingCells(car)).thenReturn(
	 * availableCells); // Assert assertEquals("Available cells", availableCells,
	 * vigilantManagerImpl.getAvailableParkingCells(car)); }
	 */
	...
	/*
	 * @Test public void updateCellStatusTest() { // Arrange ParkedVehicle
	 * parkedVehicle = new ParkedVehicleDataBuilder().build();
	 * 
	 * // Act vigilantManagerImpl.updateCellStatus(parkedVehicle); // Assert }
	 */

	/*
	 * @Test public void updateParkedVehiclesTest() { // Arrange ParkedVehicle
	 * parkedVehicle = new ParkedVehicleDataBuilder().build();
	 * 
	 * // Act vigilantManagerImpl.updateParkedVehicles(parkedVehicle); // Assert }
	 */

	/*
	 * @Test public void updateVehicleOutTest() { // Arrange Long chargedFee =
	 * 5000L; // Act
	 * 
	 * vigilantManagerImpl.updateVehicleOut(chargedFee); // Assert }
	 */
